...
void LineFeatureTracker::readIntrinsicParameter(const string &calib_file)
{
    ...
    if (!fs["model_type"].isNone())
    {
        ...
        std::cout << "model: " << sModelType << std::endl;
        if (sModelType.compare("PINHOLE") != 0)
        {
            std::cout << "do not find a pinhole model" << std::endl;
            return;
        }
    }

    std::cout << "Pin-hole model" << std::endl;
...
void LineFeatureTracker::readImage(const cv::Mat &_img)
{
    ...
    TicToc t_p;
    frame_cnt++;
//    if(pinhole == nullptr)  // other camera model
...
//        /* compute matches */
//        TicToc t_match;
//        line_tracker::LineMatcher matcher;
//        std::vector<bool> kl_status;
//        std::vector<KeyLine> optflow_keylsd;
//        matcher.opticalFlowBasedLineMatch(curframe_->img, forwframe_->img, curframe_->keylsd, optflow_keylsd, kl_status);
//        ROS_INFO("t_match costs: %fms", t_match.toc());
//
//        for (int j = 0; j < kl_status.size(); ++j) {
//            if(kl_status.at(j))
//            {
//                forwframe_->keylsd.push_back( optflow_keylsd[j] );
//                int id = curframe_->lineID[j];
//                forwframe_->lineID.push_back(id);
//            }
//        }
//
//        visualize_line_match(forwframe_->img.clone(), curframe_->img.clone(), optflow_keylsd, curframe_->keylsd, kl_status);
//
//        int diff_n = 50 - forwframe_->keylsd.size();  // 跟踪的线特征少于50了，那就补充新的线特征, 还差多少条线
//        if( diff_n > 0)    // 补充线条
//        {
//            std::vector<KeyLine> lsd, keylsd;
//            Ptr<LSDDetector> lsd_;
...
//                forwframe_->keylsd.push_back(keylsd[k]);
//                forwframe_->lineID.push_back(allfeature_cnt++);
//            }
//
//        }
