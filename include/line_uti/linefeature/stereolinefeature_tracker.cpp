...
stereoLineFeatureTracker::stereoLineFeatureTracker()
{
    allfeature_cnt = 0;
    frame_cnt = 0;
}

void stereoLineFeatureTracker::readIntrinsicParameter(const string &calib_file)
{
    ...
    float m_k1, m_k2, m_p1, m_p2, m_fx, m_fy, m_cx, m_cy;
    int m_imageWidth,m_imageHeight;
    m_imageWidth = static_cast<int>(fs["image_width"]);
    m_imageHeight = static_cast<int>(fs["image_height"]);

    cv::FileNode n = fs["stereo_distortion_parameters"];
    m_k1 = static_cast<double>(n["k1"]);
    m_k2 = static_cast<double>(n["k2"]);
    m_p1 = static_cast<double>(n["p1"]);
    m_p2 = static_cast<double>(n["p2"]);

    n = fs["stereo_projection_parameters"];
    m_fx = static_cast<double>(n["fx"]);
    m_fy = static_cast<double>(n["fy"]);
    m_cx = static_cast<double>(n["cx"]);
    m_cy = static_cast<double>(n["cy"]);

    pinhole = new Camera_Model::PinholeCamera( (double)m_imageWidth, (double)m_imageHeight,
                                               m_fx, m_fy, m_cx, m_cy,
                                               m_k1, m_k2, m_p1, m_p2);
    cv::Mat K(cv::Size(3,3), CV_32FC1);
    K.at<float>(0, 0) = static_cast<float>(m_fx);
    K.at<float>(1, 1) = static_cast<float>(m_fy);
    K.at<float>(0, 2) = static_cast<float>(m_cx);
    K.at<float>(1, 2) = static_cast<float>(m_cy);
    K_= K;
    std::cout << " pinhole Camera_Model" << std::endl;
    std::cout << " K_  = " << K_ << std::endl;

}

vector<Line> stereoLineFeatureTracker::undistortedLineEndPoints(vector<Line> & un_lines_R)
{
    ...
    un_lines_R = curframe_->vecLine_matched_R;
    for (unsigned int i = 0; i <curframe_->vecLine_matched_R.size(); i++)
    {
        un_lines_R[i].StartPt.x = (curframe_->vecLine_matched_R[i].StartPt.x - cx)/fx;
        un_lines_R[i].StartPt.y = (curframe_->vecLine_matched_R[i].StartPt.y - cy)/fy;
        un_lines_R[i].EndPt.x = (curframe_->vecLine_matched_R[i].EndPt.x - cx)/fx;
        un_lines_R[i].EndPt.y = (curframe_->vecLine_matched_R[i].EndPt.y - cy)/fy;
    }
    return un_lines;
}

void stereoLineFeatureTracker::NearbyLineTracking(const vector<Line> forw_lines, const vector<Line> cur_lines,
...
//#define NLT
#ifdef  NLT
void stereoLineFeatureTracker::readImage(const cv::Mat &_img)
{
    cv::Mat img;
    TicToc t_p;
    frame_cnt++;
    pinhole->undistortImage(_img, img);
...
void stereoLineFeatureTracker::readImage(const cv::Mat &_img)
{
    ...
    TicToc t_jd;
    junctionDetector jd;
    ROS_INFO("jd inition costs: %fms", t_jd.toc());
    TicToc t_jd_detec;
    jd.set(forwframe_->vecLine, img.rows, img.cols);
    jd.detect();
    forwframe_->Junc = jd.get();
    ROS_INFO("junc detect costs: %fms", t_jd_detec.toc());

    TicToc t_jd_des;
    jd.runDescriptor(img);
    Mat desc;
    vector<KeyPoint> kp;
    jd.getDescriptor(kp, desc);
    forwframe_->kp = kp;
    forwframe_->JuncDesc = desc;
    ROS_INFO("junc desc costs: %fms", t_jd_des.toc());
    ...
    if(curframe_->vecLine.size() > 0)
    {
        ...
        TicToc t_lmini;
        int descriptorMethod=0;
        int matchMethod =0;
        bool enableRansac = false;
        bool isStereoPair = false;
        float minLength = 40;
        lineMatcher lm(descriptorMethod, matchMethod, enableRansac, isStereoPair, minLength);
        ROS_INFO("match inition costs: %fms", t_lmini.toc());
        TicToc t_lm;
        lm.set(curframe_->Junc, forwframe_->Junc, curframe_->img, forwframe_->img,
               curframe_->kp, forwframe_->kp, curframe_->JuncDesc, forwframe_->JuncDesc,
               curframe_->vecLine.size(), forwframe_->vecLine.size());
        lm.lineMatch();
        Mat F;
        vector<pair<int, int> > juncMatches, lineMatches;
        lm.get(lineMatches, juncMatches, F);
        ROS_INFO("line match costs: %fms", t_lm.toc());

        // show zj match
        for(int j = 0; j < lineMatches.size(); j++)
        {
            forwframe_->lineID[lineMatches[j].second] = curframe_->lineID[lineMatches[j].first];
        }

        // show NLT match
        visualizeLineMatch(curframe_->vecLine, forwframe_->vecLine, lineMatches,
                           curframe_->img, forwframe_->img, "Putative Line Matches", 10, true,
                           "frame");
        visualizeLinewithID(forwframe_->vecLine,forwframe_->lineID,forwframe_->img,"forwframe_");
        visualizeLinewithID(curframe_->vecLine,curframe_->lineID,curframe_->img,"curframe_");
        stringstream ss;
...
void visualize_stereoline_match(Mat imageMat1, Mat imageMat2,
...
void stereoLineFeatureTracker::readImage(const cv::Mat &_imgL, const cv::Mat &_imgR)
{
    cv::Mat imgL,imgR;
    TicToc t_p;
    frame_cnt++;
...
//    cv::waitKey(1);
    ...
    {
        cv::Ptr<cv::CLAHE> clahe = cv::createCLAHE(3.0, cv::Size(8, 8));
        clahe->apply(_imgL, imgL);
        clahe->apply(_imgR, imgR);
    }

    bool first_img = false;
    if (forwframe_ == nullptr) // 系统初始化的第一帧图像
    {
        forwframe_.reset(new stereoFrameLines);
        curframe_.reset(new stereoFrameLines);
        forwframe_->imgl = imgL;
        forwframe_->imgr = imgR;
        curframe_->imgl = imgL;
        curframe_->imgr = imgR;
        first_img = true;
    }
    ...
    {
        forwframe_.reset(new stereoFrameLines);  // 初始化一个新的帧
        forwframe_->imgl = imgL;
        forwframe_->imgr = imgR;
    }
    ...
    // step 1: line extraction
    TicToc t_li;
    std::vector<KeyLine> lsdL,lsdR, keylsdL,keylsdR ,keylsdL_matchLR,keylsdR_matchLR;
    Ptr<LSDDetector> lsd_detector;
    lsd_detector = cv::line_descriptor::LSDDetector::createLSDDetector();
    lsd_detector->detect( imgL, lsdL, 2, 2 );
    lsd_detector->detect( imgR, lsdR, 2, 2 );
   ...
   // ROS_INFO("line detect costs: %fms", t_li.toc());
    ...
    Mat lbd_descr_L, keylbd_descr_L, keylbd_descr_matchLR_L;
    Mat lbd_descr_R, keylbd_descr_R;
    //bd_ = BinaryDescriptor::createBinaryDescriptor(  );
    bd_->compute( imgL, lsdL, lbd_descr_L );
    bd_->compute( imgR, lsdR, lbd_descr_R );

//////////////////////////
    for ( int i = 0; i < (int) lsdL.size(); i++ )
    {

        if( lsdL[i].octave == 0 && lsdL[i].lineLength >= 30)
        {
            keylsdL.push_back( lsdL[i] );
            keylbd_descr_L.push_back( lbd_descr_L.row( i ) );
        }
    }
    for ( int i = 0; i < (int) lsdR.size(); i++ )
    {

        if( lsdR[i].octave == 0 && lsdR[i].lineLength >= 30)
        {
            keylsdR.push_back( lsdR[i] );
            keylbd_descr_R.push_back( lbd_descr_R.row( i ) );
        }
    }
///////////////
...
//    if(first_img)
    {
        ...
        bdm_->match(keylbd_descr_L, keylbd_descr_R, lsd_matches);

        /* select best matches */
        std::vector<DMatch> good_matchesLR;
        std::vector<KeyLine> good_KeylinesLR;
        ...
        for ( int i = 0; i < (int) lsd_matches.size(); i++ )
        {
            if( lsd_matches[i].distance < MATCHES_DIST_THRESHOLD ){
                ...
                KeyLine line1 =  keylsdL[mt.queryIdx] ;
                KeyLine line2 =  keylsdR[mt.trainIdx] ;
                ...
                if((serr.dot(serr) < 60 * 60) && (eerr.dot(eerr) < 60 * 60))   // 线段在图像里不会跑得特别远
                    good_matchesLR.push_back( lsd_matches[i] );
            }
        }

        for (int k = 0; k < good_matchesLR.size(); ++k) {
            DMatch mt = good_matchesLR[k];

            keylsdL_matchLR.push_back( keylsdL[mt.queryIdx] );
            keylsdR_matchLR.push_back( keylsdR[mt.trainIdx] );
            keylbd_descr_matchLR_L.push_back( keylbd_descr_L.row( mt.queryIdx ) );

//            forwframe_->lineID[mt.queryIdx] = curframe_->lineID[mt.trainIdx];

        }
//        visualize_stereoline_match(imgL.clone(), imgR.clone(), keylsdL, keylsdR, good_matchesLR);

    }

    forwframe_->keylsd = keylsdL_matchLR;
    forwframe_->keylsd_R = keylsdR_matchLR;
    forwframe_->lbd_descr = keylbd_descr_matchLR_L;
    ...
    if(curframe_->keylsd.size() > 0)
    {
        ...
        ROS_INFO("lbd_macht costs: %fms", t_lbd.toc());
...
//        ROS_INFO("11111111");
        ...
        visualize_stereoline_match(forwframe_->imgl.clone(), curframe_->imgl.clone(), forwframe_->keylsd, curframe_->keylsd, good_matches);

        vector<KeyLine> vecLine_tracked, vecLine_new, vecLine_tracked_R, vecLine_new_R;
        vector< int > lineID_tracked, lineID_new;
        ...
        for (size_t i = 0; i < forwframe_->keylsd.size(); ++i)
        {
            if( forwframe_->lineID[i] == -1)
            {
                ...
                vecLine_new_R.push_back(forwframe_->keylsd_R[i]);
                lineID_new.push_back(forwframe_->lineID[i]);
            ...
            else
            {
                ...
                vecLine_tracked_R.push_back(forwframe_->keylsd_R[i]);
                lineID_tracked.push_back(forwframe_->lineID[i]);
        ...
        if( diff_n > 0)    // 补充线条
        {
            for (int k = 0; k < vecLine_new.size(); ++k) {
                vecLine_tracked.push_back(vecLine_new[k]);
                vecLine_tracked_R.push_back(vecLine_new_R[k]);
                lineID_tracked.push_back(lineID_new[k]);
        ...
        forwframe_->keylsd_R = vecLine_tracked_R;
        forwframe_->lineID = lineID_tracked;
    ...
    for (int j = 0; j < forwframe_->keylsd.size(); ++j) {
        ...
        // 右目
        lsd = forwframe_->keylsd_R[j];
        l.StartPt = lsd.getStartPoint();
        l.EndPt = lsd.getEndPoint();
        l.length = lsd.lineLength;
        forwframe_->vecLine_matched_R.push_back(l);
    }
    curframe_ = forwframe_;
