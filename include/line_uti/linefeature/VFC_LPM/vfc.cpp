...
Point2f VFC::calculateOneVec(Point2f Pt) {
	float t1 = 0;
	float t2 = 0;
	if (_method == NORMAL_VFC) {
		for (int j = 0; j < _numPt; j++) {
			float t = pow(Pt.x - _X[j].x, 2) + pow(Pt.y - _X[j].y, 2);
			float p = expf(-_beta * t);
			t1 += p * _C[j].x;
			t2 += p * _C[j].y;
		}
		...
		for (int j = 0; j < _numCtrlPts; j++) {
			float t = pow(Pt.x - _ctrlPts[j].x, 2) + pow(Pt.y - _ctrlPts[j].y, 2);
			float p = expf(-_beta * t);
			t1 += p * _C[j].x;
			t2 += p * _C[j].y;
		}
	}
	return Point2f(t1, t2);
}

vector<Point2f> VFC::denseVecFld(vector<Point2f> samplePts)
{
	int N = samplePts.size();
	vector<Point2f> vecFld;
	vecFld.reserve(N);
	for (int i = 0; i < N; i++) {
		Point2f P = samplePts[i];
		float x = (P.x - _meanLeftX.x) / _scaleLeftX;
		float y = (P.y - _meanLeftX.y) / _scaleLeftX;
		Point2f V = calculateOneVec(Point2f(x, y));
		V.x = (V.x + x) * _scaleRightX + _meanRightX.x;
		V.y = (V.y + y) * _scaleRightX + _meanRightX.y;
		vecFld.push_back(V);
	}
	return vecFld;
}