...
Vector6d gc_line_to_pose(Vector6d line_w, pose_t T) {
  ...
  Vector3d cp1 = gc_point_to_pose( T, cp0 );   // Rji * ni + [tji]x * Rji * vi, 这里不是采用的这个公式呀，而是对线段的原点变换
  Vector3d dv1 = T.R * dv0;                    // Rji * vi
...
Vector6d gc_plk_to_pose( Vector6d plk_w, pose_t T ) {
  ...
  Vector3d nc = T.R * nw + gc_skew_symmetric(T.t) * T.R * vw;   // Rji * ni + [tji]x * Rji * vi
  Vector3d vc = T.R * vw;
